...
[issue](https://github.com/inclusive-design/guide.inclusivedesign.ca/issues) with the new localizations in the description, comments, or file attachments of the issue. When another
contributor is available, they could work with the original contributor to shepherd the localization into the project.
...
separated into collections/directories for the various categories (i.e. `activities/`, `insights/`, `tools/`, `practices/`) and pages
(i.e. `index.md`, `404.md` in for each language in the `pages` directory).
...
locale. e.g. `src/documents/activities/en-CA/MyActivity.md` and `src/documents/activities/fr-FR/MyActivity.md`. They both
...
While most of the localizable text is contained within the Markdown `.md` files located in `./src/documents/` , there are other text strings that need localization like the header, the footer, aria-labels, and navigation. These strings can be found in the following locations:
...
The Nunjucks templates located in [./src/_includes/layouts/layouts/](./src/_includes/layouts) and [./src/_includes/partials/](./src/_includes/partials) use ways to include translatable strings via a function call:
* `{{ _() }}` - takes a `key` which is the text to be translated. Example: `{{ _("Home") }}`.
* `{{ _i() }}` - allows for interpolation of values into the translated string. Example `{{ _i("licensed under a ${license}.", {license: license}) | safe }}`.
...
In JavaScript, import the [eleventy-plugin-i18n-gettext](https://www.npmjs.com/package/eleventy-plugin-i18n-gettext) the code and make calls to the functions.
...
_**NOTE**: Ensure that `key`  is plain text when using gettext function calls, otherwise it will not be included in the `messages.js` file which is used by .PO editors.  See [gettext issue #23](https://github.com/sgissinger/eleventy-plugin-i18n-gettext/issues/23). As a workaround for JavaScript and templates, resolve the `key` variable elsewhere. For example the category names used in URLs are explicitly called in
`src/_data/site.js`._

The specific localization strings are provided in the `.po` and/or `.mo` files locataed in the `[./src/locales/](./src/locales)` directory. The strings to translate are automatically added to [messages.js](./src/locales/messages.js) when a build is generated (i.e. `npm start`). This also allows translation tools like [Poedit](https://poedit.net) to facilitate editing strings that need to be translated. Alternatively, `.po` files can be manually edited and `.mo` files generated using other tools (i.e. `msgfmt` command-line tool).
...
3. Select `Translations > Update from Source Code`.
   ...
   * If this step does not find the available localizations from the `./src/locales/messages.js` file, ensure that the source paths are set correctly: `Translations > Properties... > Source Paths`.
...
7. Save.
8. Make sure to commit the updates `.po` and `.mo` files to the working repository.
